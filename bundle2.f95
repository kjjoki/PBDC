      MODULE bundle2
        ...
        !| |    DELETE FROM BUNDLE           : delete_b2(set, eps)                            | |
        ...
        !| |                                                                                  | |
        !| |                                                                                  | |
        !| |    PRINTS THE BUNDLE               : information_b2(set)                         | |       
        !| |    PRINTS THE DIRECTIONS           : information_directions(set)                 | |
        !| |                                                                                  | |
        !| .**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**. |
        !*..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..*       
        ...
        !**************************************************************************************
           ...
           SUBROUTINE delete_b2(set, eps)
               !
               ! Deletes from the bundle 'set' those elements for which the linearization error 'alpha' > 'eps'. 
               !
               ...
               k = 1 
               up = set%b_size   ! the current size of the bundle
               ...
               DO j = 1, up 
                   IF( set%b_elements(j)%lin_error + 0.000000001_dp <= eps ) THEN
                      IF (j /= k ) THEN 
                          ...
                          ! we don't transfer the information about the solutions related to the subproblem j
                          ! because that information has to be recalculated in the next round 
                          ! (this is due to the change of the parameter value t)
                      END IF
                      k = k + 1 
                   END IF
               END DO
               
               IF (set%b_size /= k - 1) THEN                ! is executed when something is deleted/removed from the bundle 
                   set%b_size = k - 1                       ! the new number of stored subgradients
                   set%indeksi = k                          ! the place where the new bundle element tried to be added
        ...
        !*************************************************************************************
        ! ********************************************************************************** |
        ! |                      PRINTS THE BUNDLE AND THE DIRECTIONS                      | |
        ! ********************************************************************************** |
        !*************************************************************************************      
           
           SUBROUTINE information_b2(set)
               !
               ...
               WRITE(*,*) 'Bundle B_2 contains elements: '
           ...
           END SUBROUTINE information_b2
           

           SUBROUTINE information_directions(set)
               !
               ! Prints the information about the directions in the bundle 'set'.
               !
               IMPLICIT NONE
               !**************************** NEEDED FROM USER *************************************
               TYPE(kimppu2), INTENT(IN) :: set ! bundle
               ...
               WRITE(*,*) 'Bundle B_2 contains directions: '
               WRITE(*,*) ' '              
               WRITE(*,*) 'Direction, Change and Function value of the subproblem'
               DO i = 1, set%b_size  
                  
                   WRITE(*,*) '( [', set%b_elements(i)%direction, '] , (' , &
                        & set%b_elements(i)%change, &
                        & set%b_elements(i)%subprob_value, ' )'                     
               END DO
            
               
               WRITE(*,*) 'and also ( [', set%current_element%direction, '] , (', &
                        & set%current_element%change, &
                        & set%current_element%subprob_value, ' )'
               WRITE(*,*) ' '                       
           
           END SUBROUTINE information_directions
