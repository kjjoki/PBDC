      MODULE bundle1  
        ...
        !| |    ADD&GIVE ELEMENT       : add_give_b1(set, grad, alpha, old_grad, old_alpha)   | |
        ...
        !| |                                                                                  | |       
        !| |                                                                                  | |
        !| |    PRINTS THE BUNDLE           : information_b1(set)                             | |       
        !| |                                                                                  | |       
        !| .**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**. |
        !*..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..**..*   
        ...
        !**************************************************************************************
        !                                                                                     |
        !         ADD ELEMENT INTO TO THE BUNDLE & GIVES VALUES OF OVERWRITTEN ELEMENT        | 
        !                                                                                     |
        !**************************************************************************************    
  
           SUBROUTINE add_give_b1(set, grad, alpha, old_grad, old_alpha)
               !
               ! Adds the element '(grad, alpha)' into the bundle 'set' (i.e. into the bundle element table 'b_elements')
               ! and gives the overwritten bundle element into the values 'old_grad' and 'old_alpha.
               !
               ! NOTICE: the dimension of vectors 'grad' and 'old_grad' has to be 'set%n'.
               !
               IMPLICIT NONE
               !**************************** NEEDED FROM USER *************************************
               TYPE(kimppu1), INTENT(INOUT) :: set                      ! bundle
               REAL(KIND=dp), DIMENSION(set%n), INTENT(IN):: grad       ! the new subgradient which is added into the table 'b_elements' 
               REAL(KIND=dp), DIMENSION(set%n), INTENT(OUT):: old_grad  ! the subgradient of the overwritten element               
               REAL(KIND=dp), INTENT(IN) :: alpha                       ! linearization error which corresponds to the new subgradient
               REAL(KIND=dp), INTENT(OUT) :: old_alpha                  ! linearization error of the overwritten element
               !**************************** OTHER VARIABLES **************************************
               INTEGER :: i
               ...
               IF (set%b_maxsize > 0 ) THEN ! executed if bundle is larger than 1 (i.e. something can be stored into the table 'b_elements')
                   ...
                   old_grad = set%b_elements(i)%subgrad    ! the subgradient of the overwritten element
                   old_alpha = set%b_elements(i)%lin_error ! the linearization error of the overwritten element
                   
                   set%b_elements(i)%subgrad = grad     ! adds the new subgradient into the position i
                   set%b_elements(i)%lin_error = alpha  ! adds the new linearization error into the position i
                   set%indeksi = i + 1                  ! the position where the next element is added
                  
                  IF ( .NOT. set%full ) THEN            ! if the bundle was not full during previous round, then the size of the bundle is increased with 1
                       set%b_size = set%b_size + 1
                   END IF
        ...
        !**************************************************************************************
           ...
           PURE FUNCTION grad_matrix_agg(set) RESULT(m)
               ...
               !**************************** NEEDED FROM USER *************************************
               TYPE(kimppu1), INTENT(IN) :: set                     ! bundle
               REAL(KIND=dp), DIMENSION(set%n*(set%b_size+1)) :: m  ! subgradient matrix formed from the bundle 'set'
               !**************************** OTHER VARIABLES **************************************            
               INTEGER :: i, j, length, start
        ...
        !*************************************************************************************  
           ...
           SUBROUTINE information_b1(set)
               !
               ! Prints the information about the bundle 'set' 
               !
               IMPLICIT NONE
               !**************************** NEEDED FROM USER *************************************
               TYPE(kimppu1), INTENT(INOUT) :: set ! bundle
               ...
               WRITE(*,*) 'Bundle B_1 contains elements: '

               WRITE(*,*) 'The current element: '              
               WRITE(*,*) '( [', set%current_element%subgrad, '] , ' , &
                        & set%current_element%lin_error, ' )'
                
               WRITE(*,*) 'Other elements: '                
               DO i = 1, set%b_size 
                  
                   WRITE(*,*) '( [', set%b_elements(i)%subgrad, '] , ' , &
                        & set%b_elements(i)%lin_error, ' )'
               END DO       

               IF (set%agg) THEN
                   WRITE(*,*) 'The aggregated element:  ' 
                   WRITE(*,*) '( [', set%agg_element%subgrad, '] , ' , &
                        & set%agg_element%lin_error, ' )'              
               END IF 
               
               WRITE(*,*) ' '              
               
           END SUBROUTINE information_b1
